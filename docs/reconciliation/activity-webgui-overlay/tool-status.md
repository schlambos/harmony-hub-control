...
> Point-in-time record of the source-only `activity-webgui` overlay worktree
> (HEAD `391e10e…`). Live box binary identities remain product truth. The
> current product build flow is in `docs/BUILD.md` and
