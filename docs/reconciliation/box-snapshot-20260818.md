...
(publication-hygiene remediations applied; canonical blockers retained).
Offline and deterministic. No payload/source/build files were modified; no
private evidence objects were copied into the repository.
...
- **112 unique commits** across the union of all refs in both repos:
  historical clone contributes 57, baseline contributes 89, **overlap 34**
  (55 commits are baseline-unique, 23 historical-unique) — 57 + 89 − 34 =
  112. Independently re-derived from git in the test suite and pinned as a
  regression constant.
...
| /data/codex/bin/codex_bt_pair_agent | file | 111796 | CANDIDATE_SOURCE_BINARY_MATCH_ONLY | HISTORICAL_BINARY_MATCH_ONLY | yes | 54 | e512cd4641b6… |
| /data/codex/bin/codex_bthid_keyboard | file | 116452 | CANDIDATE_SOURCE_BINARY_MATCH_ONLY | HISTORICAL_BINARY_MATCH_ONLY | yes | 54 | 47d0bd1eb7ff… |
| /data/codex/bin/codex_dhcpd | file | 104168 | CANDIDATE_SOURCE_BINARY_MATCH_ONLY | HISTORICAL_BINARY_MATCH_ONLY | yes | 111 | 48070f511298… |
| /data/codex/bin/codex_hal_ltcp | file | 76772 | CANDIDATE_SOURCE_BINARY_MATCH_ONLY | HISTORICAL_BINARY_MATCH_ONLY | yes | 54 | 119eeed9a122… |
| /data/codex/bin/codex_hbus | file | 74660 | CANDIDATE_SOURCE_NO_BINARY_MATCH | **RECIPE_UNPROVEN** | no | — | — |
| /data/codex/bin/codex_portal | file | 109548 | CANDIDATE_SOURCE_BINARY_MATCH_ONLY | HISTORICAL_BINARY_MATCH_ONLY | yes | 111 | a745df84f2ca… |
...
| /opt/luaworks/tasks/connectserver/netservicestarter.lua | file | 12707 | **MANUAL_SOURCE_REQUIRED** + **PUBLIC_SAFETY_PASS** | n/a | no | — | — |
...
| /usr/sbin/dropbear | file | 52 | MANUAL_SOURCE_REQUIRED (near-miss) | n/a | no | — | — |
...
### netservicestarter.lua — DIAG patch, manual source required
...
equals the committed clean blob lineage. Source provenance:
`MANUAL_SOURCE_REQUIRED`.

### /usr/sbin/dropbear — near-miss wrapper
...
exactly by the `-s -g` tokens → `MANUAL_SOURCE_REQUIRED`.
`/usr/sbin/dropbearkey` is an exact match across 111 commits.
...
- The DIAG netservicestarter live source bytes: **PUBLIC_SOURCE_SAFE** (its
  *source provenance* remains MANUAL_SOURCE_REQUIRED and is tracked
  separately — the safety verdict makes no provenance or reproducibility
  claim).
...
`build_verified_count` is **0**; every matched binary carries
`HISTORICAL_BINARY_MATCH_ONLY` (or `UNVERIFIED_THIRD_PARTY` for dropbearmulti,
`RECIPE_UNPROVEN` for hbus). Tests fail if any such claim appears.

## Blockers (machine-readable in `reproducibility-status.json`)

1. `MANUAL_SOURCE_REQUIRED` — netservicestarter.lua (DIAG variant) and
   /usr/sbin/dropbear (near-miss wrapper).
2. `NO_BINARY_BUILD_REPRODUCIBILITY` — zero verified binary rebuilds; hbus
   recipe explicitly unproven.
3. `LIVE_MANIFEST_STALE` — 4 hash/size mismatches, 1 omission.

Resolved by remediation: `HISTORY_GAP` (union baseline resolves `1a9e270…`);
`PUBLIC_SAFETY_PENDING` (review completed → PUBLIC_SAFETY_PASS).
...
The provenance suite (45 tests, ResourceWarning-clean under
...
DIAG statuses (MANUAL_SOURCE_REQUIRED + PUBLIC_SAFETY_PASS); safety-review
...
no-reproduction-claim-from-binary-match invariant; and **publication
hygiene** (every committed deliverable under `tools/reconciliation`,
